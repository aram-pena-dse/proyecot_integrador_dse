...
void main_thread_entry(void) {
   ...
   gx_widget_attach(p_window_root, p_first_screen);
    ...
    GX_PROMPT * p_prompt;
    GX_PROMPT * p_prompt_sp;
    GX_PROMPT * p_prompt_dc;

    static uint32_t index = 5;

//    static GX_CHAR txt_buffer[4] = "1";
    static GX_CHAR target_temp_string[10];

    gx_utility_ltoa(index++, (GX_CHAR *)target_temp_string, 10);

    //status = gx_system_widget_find(ID_WIN1_RPM,3, (GX_WIDGET **) &p_prompt);
    gx_widget_find(p_window_root, ID_WIN1_RPM, 3, (GX_WIDGET **) &p_prompt);
    gx_widget_find(p_window_root, ID_WIN1_SP, 3, (GX_WIDGET **) &p_prompt_sp);
    gx_widget_find(p_window_root, ID_WIN1_DC, 3, (GX_WIDGET **) &p_prompt_dc);

    /* Attach txt_buffer to txt_counter widget */
    //status = gx_prompt_text_set(p_prompt, txt_buffer);
    gx_prompt_text_set(p_prompt, (GX_CHAR *)target_temp_string);
    gx_prompt_text_set(p_prompt_sp, (GX_CHAR *)target_temp_string);
    gx_prompt_text_set(p_prompt_dc, (GX_CHAR *)target_temp_string);

    /* Let GUIX know that txt_counter should be re-drawn */
    status = gx_system_dirty_mark((GX_WIDGET *) p_prompt);
    status = gx_system_dirty_mark((GX_WIDGET *) p_prompt_sp);
    status = gx_system_dirty_mark((GX_WIDGET *) p_prompt_dc);
