...
TX_THREAD adc_example;
void adc_example_create(void);
static void adc_example_func(ULONG thread_input);
/** Alignment requires using pragma for IAR. GCC is done through attribute. */
#if defined(__ICCARM__)
#pragma data_alignment = BSP_STACK_ALIGNMENT
#endif
static uint8_t adc_example_stack[1024] BSP_PLACE_IN_SECTION(".stack.adc_example") BSP_ALIGN_VARIABLE(BSP_STACK_ALIGNMENT);
...
{ .p_info = &g_transfer0_info, .activation_source = ELC_EVENT_ELC_SOFTWARE_EVENT_0, .auto_enable = false, .p_callback =
          NULL,
  .p_context = &g_transfer0, .irq_ipl = (2) };
...
{ .unit = 1, .mode = ADC_MODE_SINGLE_SCAN, .resolution = ADC_RESOLUTION_12_BIT, .alignment = ADC_ALIGNMENT_RIGHT,
  ...
  .trigger_group_b = ADC_TRIGGER_SYNC_ELC, .p_callback = NULL, .p_context = &g_adc0, .scan_end_ipl = (1),
...
{ .p_lower_lvl_adc = &g_adc0, .p_lower_lvl_timer = &g_timer0, .p_lower_lvl_transfer = &g_transfer0, .p_data_buffer =
          g_user_buffer,
  .data_buffer_length = 128, .sample_count = 10, .p_callback = g_adc_framework_user_callback, .p_extend = NULL, };
...
void adc_example_create(void)
{
    ...
    tx_thread_create (&adc_example, (CHAR *) "adc_example thread", adc_example_func, (ULONG) NULL, &adc_example_stack,
                      1024, 3, 3, 100, TX_AUTO_START);
}
