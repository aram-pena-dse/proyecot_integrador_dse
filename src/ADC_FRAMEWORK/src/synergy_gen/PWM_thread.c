...
#if !defined(SSP_SUPPRESS_ISR_g_timer1) && !defined(SSP_SUPPRESS_ISR_GPT7)
...
{ .mode = TIMER_MODE_PWM, .period = 10, .unit = TIMER_UNIT_FREQUENCY_KHZ, .duty_cycle = 50, .duty_cycle_unit =
  ...
  .channel = 7, .autostart = true, .p_callback = NULL, .p_context = &g_timer1, .p_extend = &g_timer1_extend, .irq_ipl =
          (BSP_IRQ_DISABLED), };
...
void PWM_thread_create(void)
{
    ...
    tx_thread_create (&PWM_thread, (CHAR *) "PWM", PWM_thread_func, (ULONG) NULL, &PWM_thread_stack, 1024, 1, 1, 100,
                      TX_AUTO_START);
}
