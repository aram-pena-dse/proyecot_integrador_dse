...
TX_THREAD computation_thread;
void computation_thread_create(void);
static void computation_thread_func(ULONG thread_input);
/** Alignment requires using pragma for IAR. GCC is done through attribute. */
#if defined(__ICCARM__)
#pragma data_alignment = BSP_STACK_ALIGNMENT
#endif
static uint8_t computation_thread_stack[1024] BSP_PLACE_IN_SECTION(".stack.computation_thread") BSP_ALIGN_VARIABLE(BSP_STACK_ALIGNMENT);
...
void computation_thread_create(void)
{
    ...
    tx_thread_create (&computation_thread, (CHAR *) "Computation Thread", computation_thread_func, (ULONG) NULL,
                      &computation_thread_stack, 1024, 1, 1, 1, TX_AUTO_START);
}
