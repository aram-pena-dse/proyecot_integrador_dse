...
TX_THREAD sensor_thread1;
void sensor_thread1_create(void);
static void sensor_thread1_func(ULONG thread_input);
/** Alignment requires using pragma for IAR. GCC is done through attribute. */
#if defined(__ICCARM__)
#pragma data_alignment = BSP_STACK_ALIGNMENT
#endif
static uint8_t sensor_thread1_stack[1024] BSP_PLACE_IN_SECTION(".stack.sensor_thread1") BSP_ALIGN_VARIABLE(BSP_STACK_ALIGNMENT);
...
void sensor_thread1_create(void)
{
    ...
    tx_thread_create (&sensor_thread1, (CHAR *) "Sensor Thread 1", sensor_thread1_func, (ULONG) NULL,
                      &sensor_thread1_stack, 1024, 1, 1, 1, TX_AUTO_START);
}
